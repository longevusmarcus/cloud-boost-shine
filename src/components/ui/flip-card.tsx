...
export default function FlipCard({
  ...
  return (
    <div className={cn("perspective-1000", className)} style={{ width: cardWidth, height: cardHeight }}>
      ...
      >
        {/* FRONT SIDE */}
        <Card className="absolute w-full h-full backface-hidden bg-white dark:bg-gray-900 shadow-md rounded-2xl p-4 flex flex-col justify-center items-center">
          {!success ? (
            <>
              {frontIllustration ?? <div className="w-20 h-20 bg-blue-100 dark:bg-blue-900 rounded-full mb-4" />}
              <CardHeader>
            ...
            <>
              {successIllustration ?? <div className="w-20 h-20 bg-green-100 dark:bg-green-900 rounded-full mb-4" />}
              <CardHeader>
                <CardTitle className="text-lg font-semibold text-center">{successTitle}</CardTitle>
              </CardHeader>
              <CardContent className="text-center text-sm text-gray-600 dark:text-gray-400">{successDescription}</CardContent>
        ...
        {/* BACK SIDE */}
        <Card
          className="absolute w-full h-full backface-hidden bg-white dark:bg-gray-900 shadow-md rounded-2xl p-6 flex flex-col justify-center"
          style={{ transform: "rotateY(180deg)" }}
        >
          {backIllustration}
          <h3 className="text-lg font-semibold mb-2">{backTitle}</h3>
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">{backDescription}</p>
